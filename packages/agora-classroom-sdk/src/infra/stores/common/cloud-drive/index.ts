...
import { MimeTypesKind } from '@classroom/infra/utils/board-utils';
  ...
  onInstall() {
    ...
    this._disposers.push(
      reaction(
        ...
        (mounted) => {
          if (mounted) {
            const firstKey = Array.from(this.publicResources.keys())[0];
            const courseware = this.publicResources.get(firstKey);
            if (courseware) {
              this.openResource(courseware);
            }
          }
        },
      ),
