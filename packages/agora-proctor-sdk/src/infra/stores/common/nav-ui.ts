import { AgoraProctorSDK } from '@proctor/infra/api';
import { number2Percent } from '@proctor/infra/utils';
import {
  AGServiceErrorCode,
  ClassroomState,
  ClassState,
...
export class NavigationBarUIStore extends EduUIStoreBase {
  ...
  get networkQualityIcon(): { icon: SvgIconEnum; color: string } {
    switch (this.networkQuality) {
      case AGNetworkQuality.good:
        return {
          icon: SvgIconEnum.NORMAL_SIGNAL,
          color: NetworkStateColors.normal,
        };
      case AGNetworkQuality.bad:
    ...
    return {
      icon: SvgIconEnum.UNKNOWN_SIGNAL,
      color: NetworkStateColors.unknown,
    };
  }
